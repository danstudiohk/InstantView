...
var DataPath = "data/csv/Total Member.csv";//Data Source location
...
//GET MAX value to set Axis Y
	...
	svgLine_ACTmin = d3.min(filteredData_byMktAll,function(d) {
			return d.ACT_Memb;
	});

	svgLine_BGTmin = d3.min(filteredData_byMktAll,function(d) {
			return d.BGT_Memb;
	});

	svgLine_LYmin = d3.min(filteredData_byMktAll,function(d) {
			return d.LY_Memb;
	});
	...
	svgLine_Valuemin = Math.max(svgLine_ACTmin,svgLine_BGTmin,svgLine_LYmin)*0.5;
	...
	svgLine_ScaleY.domain([svgLine_Valuemin, svgLine_Valuemax + margin.top + margin.bottom ]);
...
*/
	...
	var layers = stack(nest_Market.entries(filteredData_byMkt));
