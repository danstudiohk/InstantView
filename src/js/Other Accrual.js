...
var DataPath = "data/csv/Other Accrual.csv";//Data Source location
...
function Execute(){
	...
	CatVal = document.getElementById("Category").value; //Category
...
//filtered by Market and Carrier
	filteredData_byMktCat = entireData.filter(function (df) {
	...
	filteredData_byMktCat.forEach(function(d) {
		d.Date = d.Date;
		d.Year = d.Year;
		d.Month = d.Month;
...
//filtered by Market and Carrier and Month (for Actual)
	filteredData_byMktCatMonth = entireData.filter(function (df) {
		return df.Market == KeyMkt &&
			df.Category == CatVal &&
			df.Year == numyear &&//year parameter
	...
	filteredData_byMktCatMonth.forEach(function(d) {
		d.Date = d.Date;
		d.Year = d.Year;
		d.Month = d.Month;
		d.Category = d.Category;
...
//filtered by Market and Carrier and Month (for Area Chart)
	...
	filteredData_byCatMonthTotal = entireData.filter(function (df) {
	...
	filteredData_byCatMonthTotal.forEach(function(d) {
		d.Date = d.Date;
		d.Year = d.Year;
		d.Month = d.Month;
		d.Category = d.Category;
	...
	if(DimVal == 0){
		filteredData_byMktCatMonthTotal = filteredData_byMktMonthTotal;
	} else {
		filteredData_byMktCatMonthTotal = filteredData_byCatMonthTotal;
	};
...
//filtered by Market and without Total Carrier
	...
	filteredData_byCat = entireData.filter(function (df) {
		return df.Market != "All Markets" &&
			df.Category == CatVal &&
			df.Year == numyear &&//year parameter
			df.Month <= nummonth //month parameter
	...
	filteredData_byCat.forEach(function(d) {
		d.Date = d.Date;
		d.Year = d.Year;
		d.Month = d.Month;
		d.Category = d.Category;
	...
	Category_sort = ['Gift Miles','Asia Miles Renewal In','Asia Miles Top-up','Asia Miles Transfer In','CX/KA Department Promotions'];
	Market_sort = ['Hong Kong', 'Macau', 'Taiwan','PRD','PRC','North America','Southeast Asia','Others'];
	
	
	Data_ACT_Miles_byCatYTD = d3.nest()
		.key(function(d){return d.Category;}).sortKeys(function(a,b) {return Category_sort.indexOf(a) - Category_sort.indexOf(b);})
	...
	Data_ACT_Miles_byMktYTD = d3.nest()
		...
		.entries(filteredData_byCat);	

			
	if(DimVal == 0){
		Data_ACT_Miles_byMktCatYTD = Data_ACT_Miles_byCatYTD;
	} else {
		Data_ACT_Miles_byMktCatYTD = Data_ACT_Miles_byMktYTD;
	};	
...
//Data for Table use with calculation
	...
	Table_filteredData_byMktCat = filteredData_byMktCatMonth;

	Table_filteredData_byMktCat.forEach(function(d) {
		d.Year = d.Year;
		d.Month = d.Month;
...
//GET MAX value to set Axis Y
	svgLine_ACTmax = d3.max(filteredData_byMktCat,function(d) {
		if (MetVal == "Miles") {
			return d.ACT_Miles;
		} else if (MetVal == "Sales") {
			return d.ACT_Mileage_Sales;
	...
	svgLine_BGTmax = d3.max(filteredData_byMktCat,function(d) {
		if (MetVal == "Miles") {
			return d.BGT_Miles;
		} else if (MetVal == "Sales") {
	...
	svgLine_LYmax = d3.max(filteredData_byMktCat,function(d) {
		if (MetVal == "Miles") {
			return d.LY_Miles;
		} else if (MetVal == "Sales") {
	...
	svgLine_ScaleX.domain(d3.extent(filteredData_byMktCat, function(d) { return d.Date; }));
	...
	var svgLine_DataPath_ACT = svgLine.append("path")
		...
		.attr("d", svgLine_valueline_ACT(filteredData_byMktCatMonth))
	...
	var svgLine_DataPath_BGT = svgLine.append("path")
		...
		.attr("class", "svgLine_DataPath")
		.attr("d", svgLine_valueline_BGT(filteredData_byMktCat))
		.attr("transform", "translate(" + margin.left + "," + margin.top + ")")
		.attr("fill","none")
		.attr("stroke-width",3)
		.attr("stroke","#4BACC6")
	...
	var svgLine_DataPath_LY = svgLine.append("path")
		...
		.attr("class", "svgLine_DataPath")
		.attr("d", svgLine_valueline_LY(filteredData_byMktCat))
		.attr("transform", "translate(" + margin.left + "," + margin.top + ")")
		.attr("fill","none")
		.attr("stroke-width",3)
		.attr("stroke","grey")
	...
	svgLine.selectAll("dot")
		.data(filteredData_byMktCatMonth)
		.enter()
		.append("circle")
		.attr("class", "dot")
		.attr("r", 10)
		...
		.on("mouseover", function(d) {
			...
			d3.select("#svgLine_tooltip").transition()
				...
				d3.select("#svgLine_tooltip").html(
					formatTime(d.Date) + " <br> " +
					" Sales: " + formatNum1dp2(d.ACT_Marketing_Revenue) + " <br> " +
...
*/
	...
	svgArea_ACTmax = d3.max(filteredData_byMktCatMonthTotal
	,function(d) {
		if (MetVal == "Miles") {
			return d.ACT_Miles;
		} else if (MetVal == "Sales") {
	...
	svgArea_ScaleX.domain(d3.extent(filteredData_byMktCat, function(d) { return d.Date; }));
	...
	var nest_Category = d3.nest()
		.key(function(d) { return d.Category; }).sortKeys(function(a,b) {return Market_sort.indexOf(a) - Market_sort.indexOf(b);}).sortKeys(function(a,b) {return Category_sort.indexOf(a) - Category_sort.indexOf(b);});
	var layers_Mkt = stack(nest_Category.entries(filteredData_byMkt));
	
	var nest_Market = d3.nest()
		.key(function(d) { return d.Market; }).sortKeys(function(a,b) {return Category_sort.indexOf(a) - Category_sort.indexOf(b);}).sortKeys(function(a,b) {return Market_sort.indexOf(a) - Market_sort.indexOf(b);});
	var layers_Cat = stack(nest_Market.entries(filteredData_byCat));
...
//Pie chart Setting
	...
	pieData = pie(Data_ACT_Miles_byMktCatYTD);
...
//Pie Proportion figures
	...
	svgPie_arcs.append("text")
		...
		.delay(1500)
		.text(function(d){
			var percent = Number(d.value) / d3.sum(Data_ACT_Miles_byMktCatYTD, function(d){return d.values;})*100;
...
//Sort the filtered data
	datasorted = filteredData_byMktCat.sort(function (a, b) {
...
//Table
	...
	if (MetVal == "Miles") {
	Createtabulate(Table_filteredData_byMktCat, ['Month', 'Miles' , 'Miles vs BGT', 'Miles vs LY']);
		} else if (MetVal == "Sales") {
	Createtabulate(Table_filteredData_byMktCat, ['Month', 'Mileage Sales', 'Mileage Sales vs BGT', 'Mileage Sales vs LY']);
		} else if (MetVal == "Rev") {
	Createtabulate(Table_filteredData_byMktCat, ['Month', 'Marketing Rev', 'Marketing Rev vs BGT', 'Marketing Rev vs LY']);
		}
	;
...
*/
	function ChartTitleParsing() {
		...
		if (MetVal=="Miles") {
			...
			MetValName = "Marketing Revenue ($HK)";
		};
		if (DimVal == 0) {
			DimName = "Category";
		} else {
	...
	ChartLineHeader = "Other Accrual Growth" + KeyMktName ;
	document.getElementById("ChartLineHeader").innerHTML  = ChartLineHeader;

	ChartAreaHeader = "Other Accrual Growth by " + DimName;
	document.getElementById("ChartAreaHeader").innerHTML  = ChartAreaHeader;

	ChartPieHeader = "Other Accrual " + DimName + " Distribution";
...
Legend_Category_sort = ['Gift Miles','Renewal In','Top up','Transfer In','CX/KA Promo'];


	if(DimVal == 0){
		Legend = Legend_Category_sort;
		LegendClass = "col-12";
		offset = 100;
	} else {
