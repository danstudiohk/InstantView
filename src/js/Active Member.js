...
var DataPath = "data/csv/Active Member.csv";//Data Source location
...
//filtered by Market and without Total 
	...
	filteredData_byMemb = entireData.filter(function (df) {
		return df.Market == KeyMkt &&
			df.Type != "Total Active Member" &&
			df.Year == numyear &&//year parameter
			df.Month <= nummonth //month parameter
		;
	});

	filteredData_byMemb.forEach(function(d) {
		d.Date = d.Date;
		d.Year = d.Year;
		d.Month = d.Month;
		d.Market = d.Market;
	...
	Member_sort = ['Active Member J12M','Active Member J13M+'];
	...
	Data_ACT_Miles_byMembYTD = d3.nest()
		.key(function(d){return d.Type;}).sortKeys(function(a,b) {return Member_sort.indexOf(a) - Member_sort.indexOf(b);})
		.rollup(function(d){return d3.sum(d, function(d) {return d.ACT_Memb;});})
		.entries(filteredData_byMemb);

		
	if(DimVal == 0){
		Data_ACT_Miles_byMktTypYTD = Data_ACT_Miles_byMembYTD;	
	} else {
...
*/
	...
	svgArea_ACTmax = d3.max(filteredData_byMktMonthTotal,function(d) {return d.ACT_Memb;})*1.1;
	...
	var nest_Memb = d3.nest()
		.key(function(d) { return d.Type; }).sortKeys(function(a,b) {return Member_sort.indexOf(a) - Member_sort.indexOf(b);});
	var layers_Memb = stack(nest_Memb.entries(filteredData_byMemb));	
...
//Table
	...
	// render the table(s)
	Createtabulate(Table_filteredData_byMktTyp, ['Month', '# of Member', 'vs LY']);
	;
...
Legend_Member_sort = ['J12M', 'J13M+'];

	if(DimVal == 0){
		Legend = Legend_Member_sort;
		LegendClass = "col-12";
	} else {
