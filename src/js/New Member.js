...
var DataPath = "data/csv/New Member.csv";//Data Source location
...
function Execute(){
	...
	MembVal = document.getElementById("Member").value; //Member
	MembSplitVal = document.getElementById("Member-rule").value; //Member
...
//filtered by Market and Carrier
	filteredData_byMktAll = entireData.filter(function (df) {
	...
	filteredData_byMktAll.forEach(function(d) {
		d.Date = d.Date;
		d.Year = d.Year;
		d.Month = d.Month;
		d.Market = d.Market;
		d.ACT_Memb = +d.ACT_Memb;
		d.BGT_Memb = +d.BGT_Memb;
		d.LY_Memb = +d.LY_Memb;
...
//filtered by Market and Member and Month (for Area Chart)
	filteredData_byMktMembMonthTotal = entireData.filter(function (df) {
		return df.Market == KeyMkt &&
			df.Type == MembVal &&
			df.Year == numyear &&//year parameter
			df.Month <= nummonth //month parameter
	...
	filteredData_byMktMembMonthTotal.forEach(function(d) {
		d.Date = d.Date;
		d.Year = d.Year;
		d.Month = d.Month;
		d.Market = d.Market;
	...
	filteredData_byMembTotal = entireData.filter(function (df) {
		return df.Market == KeyMkt &&
			df.Type == "Total New Member" &&
			df.Year == numyear &&//year parameter
			df.Month <= nummonth //month parameter
		;
	});

	filteredData_byMembTotal.forEach(function(d) {
		d.Date = d.Date;
		d.Year = d.Year;
		d.Month = d.Month;
		d.Market = d.Market;
...
//Group by Member YTD  for Pie Chart
//filtered by Market and without Total 
	...
	filteredData_by5050Memb = entireData.filter(function (df) {
		return df.Market == KeyMkt &&
			(df.Type == "Air Recruited (50/50)" ||  df.Type == "Non Air Recruited (50/50)" )&&
			df.Year == numyear &&//year parameter
			df.Month <= nummonth //month parameter
		;
	});

	filteredData_by5050Memb.forEach(function(d) {
		d.Date = d.Date;
		d.Year = d.Year;
		d.Month = d.Month;
		d.Market = d.Market;
	...
	filteredData_bySplitMemb = entireData.filter(function (df) {
		return df.Market == KeyMkt &&
			(df.Type == "Air Recruited (Without Split)" ||  df.Type == "Non Air Recruited (Without Split)"  ||  df.Type == "AM.com (Without Split)" ) &&
			df.Year == numyear &&//year parameter
			df.Month <= nummonth //month parameter
		;
	});

	filteredData_bySplitMemb.forEach(function(d) {
		d.Date = d.Date;
		d.Year = d.Year;
		d.Month = d.Month;
		d.Market = d.Market;
	...
	Member_sort =['Air Recruited (50/50)','Non Air Recruited (50/50)','Air Recruited (Without Split)','Non Air Recruited (Without Split)','AM.com (Without Split)'];
	...
	Data_ACT_Miles_byMktYTD = d3.nest()
		...
		.rollup(function(d){return d3.sum(d, function(d) {return d.ACT_Memb;});})
		.entries(filteredData_byMkt);
	
	Data_ACT_Miles_by5050MembYTD = d3.nest()
		.key(function(d){return d.Type;}).sortKeys(function(a,b) {return Member_sort.indexOf(a) - Member_sort.indexOf(b);})
		.rollup(function(d){return d3.sum(d, function(d) {return d.ACT_Memb;});})
		.entries(filteredData_by5050Memb);
		
	Data_ACT_Miles_bySplitMembYTD = d3.nest()
		.key(function(d){return d.Type;}).sortKeys(function(a,b) {return Member_sort.indexOf(a) - Member_sort.indexOf(b);})
		.rollup(function(d){return d3.sum(d, function(d) {return d.ACT_Memb;});})
		.entries(filteredData_bySplitMemb);
		
		
	if(DimVal == 0){
		if(MembSplitVal == 1){
			Data_ACT_Miles_byMktTypYTD = Data_ACT_Miles_by5050MembYTD;		
		} else {
			Data_ACT_Miles_byMktTypYTD = Data_ACT_Miles_bySplitMembYTD;						
		}
	} else {
		Data_ACT_Miles_byMktTypYTD = Data_ACT_Miles_byMktYTD;	

	};	

//Data for Table use with calculation

	Table_filteredData_byMktTyp = filteredData_byMktMonth;

	Table_filteredData_byMktTyp.forEach(function(d) {
		d.Year = d.Year;
		d.Month = d.Month;
		d['# of Member'] = formatNum1dp2(+d.ACT_Memb);
		d['vs BGT'] = formatPercent((+d.ACT_Memb - +d.BGT_Memb) / +d.BGT_Memb);
		d['vs LY'] = formatPercent((+d.ACT_Memb - +d.LY_Memb) / +d.LY_Memb);
	});
	
	Table_filteredData_byMktTypSplit = filteredData_byMktMonth;

	Table_filteredData_byMktTypSplit.forEach(function(d) {
		d.Year = d.Year;
		d.Month = d.Month;
		d['# of Member'] = formatNum1dp2(+d.ACT_Memb);
		d['vs LY'] = formatPercent((+d.ACT_Memb - +d.LY_Memb) / +d.LY_Memb);
...
//GET MAX value to set Axis Y
	svgLine_ACTmax = d3.max(filteredData_byMktAll,function(d) {
			return d.ACT_Memb;
	});

	svgLine_BGTmax = d3.max(filteredData_byMktAll,function(d) {
			return d.BGT_Memb;
	});

	svgLine_LYmax = d3.max(filteredData_byMktAll,function(d) {
			return d.LY_Memb;
	});
	...
	svgLine_ScaleX.domain(d3.extent(filteredData_byMktAll, function(d) { return d.Date; }));
	...
	var svgLine_valueline_ACT = d3.svg.line()
		...
		.y(function(d) {
				return svgLine_ScaleY(d.ACT_Memb);
				})
	...
	var svgLine_valueline_BGT = d3.svg.line()
		.x(function(d) { return svgLine_ScaleX(d.Date); })
		.y(function(d) {
				return svgLine_ScaleY(d.BGT_Memb);
		})
		.interpolate('cardinal')
	;
	...
	var svgLine_valueline_LY = d3.svg.line()
		.x(function(d) { return svgLine_ScaleX(d.Date); })
		.y(function(d) {
				return svgLine_ScaleY(d.LY_Memb);
		})
		.interpolate('cardinal')
	;
	//Actual Data Path
	...
	var svgLine_DataPath_BGT = svgLine.append("path")
		...
		.attr("class", "svgLine_DataPath")
		.attr("d", svgLine_valueline_BGT(filteredData_byMktAll))
		.attr("transform", "translate(" + margin.left + "," + margin.top + ")")
		.attr("fill","none")
		.attr("stroke-width",3)
		.attr("stroke","#4BACC6")
	...
	var svgLine_DataPath_LY = svgLine.append("path")
		...
		.attr("class", "svgLine_DataPath")
		.attr("d", svgLine_valueline_LY(filteredData_byMktAll))
		.attr("transform", "translate(" + margin.left + "," + margin.top + ")")
		.attr("fill","none")
		.attr("stroke-width",3)
		.attr("stroke","grey")
	...
	svgLine.selectAll("dot")
		...
		.attr("cy", function(d) { return svgLine_ScaleY(d.ACT_Memb); })
		.attr("transform", "translate(" + margin.left + "," + margin.top + ")")
		.on("mouseover", function(d) {
			...
			d3.select("#svgLine_tooltip").transition()
				...
				d3.select("#svgLine_tooltip").html(
					...
					"Member: " + formatNum1dp2(d.ACT_Memb) + " <br> " +
					" vs BGT: " + formatPercent((d.ACT_Memb-d.BGT_Memb)/d.BGT_Memb) + " <br> " +
					" vs LY: " + formatPercent((d.ACT_Memb-d.LY_Memb)/d.LY_Memb)
					);
		...
		if (MembVal == "Air Recruited (Without Split)" ||  MembVal == "Non Air Recruited (Without Split)"  ||  MembVal == "AM.com (Without Split)"){
			$('#svgLine_DataPath_BGT').hide();
		} else {
			$('#svgLine_DataPath_BGT').show();
		};
...
*/
	...
	svgArea_ACTmax = d3.max(filteredData_byMktMembMonthTotal,function(d) {return d.ACT_Memb;})*1.1;
	...
	svgArea_ScaleX.domain(d3.extent(filteredData_byMktAll, function(d) { return d.Date; }));
	...
	var stack = d3.layout.stack()
		...
		.y(function(d) { return d.ACT_Memb; });

	var nest_Market = d3.nest()
		.key(function(d) { return d.Market; }).sortKeys(function(a,b) {return Market_sort.indexOf(a) - Market_sort.indexOf(b);});
	var layers_Mkt = stack(nest_Market.entries(filteredData_byMkt));

	var nest_5050Memb = d3.nest()
		.key(function(d) { return d.Type; }).sortKeys(function(a,b) {return Member_sort.indexOf(a) - Member_sort.indexOf(b);});
	var layers_5050Memb = stack(nest_5050Memb.entries(filteredData_by5050Memb));
		
	var nest_SplitMemb = d3.nest()
		.key(function(d) { return d.Type; }).sortKeys(function(a,b) {return Member_sort.indexOf(a) - Member_sort.indexOf(b);});
	var layers_SplitMemb = stack(nest_SplitMemb.entries(filteredData_bySplitMemb));
		
	if(DimVal == 0){
		if(MembSplitVal == 1){
			layers = layers_5050Memb;
		} else {
			layers = layers_SplitMemb;
		}
	} else {
		layers = layers_Mkt;
	};	
...
//Pie chart Setting
	...
	pieData = pie(Data_ACT_Miles_byMktTypYTD);
...
//Pie Proportion figures
	...
	svgPie_arcs.append("text")
		...
		.delay(1500)
		.text(function(d){
			var percent = Number(d.value) / d3.sum(Data_ACT_Miles_byMktTypYTD, function(d){return d.values;})*100;
			return percent.toFixed(1) +"%";})
		;

		$('.pieProportionFig').each(function() {
...
//Sort the filtered data
	datasorted = filteredData_byMktAll.sort(function (a, b) {
...
//Get YTD Figures
	...
	for (i = 0; i < 12; i++) {
			 sumBGTFY += datasorted[i].BGT_Memb;
		};
	};
	BGTFYfigure();
...
//Table
	...
	// render the table(s)
	if (MembVal == "Air Recruited (Without Split)" ||  MembVal == "Non Air Recruited (Without Split)"  ||  MembVal == "AM.com (Without Split)"){
		Createtabulate(Table_filteredData_byMktTypSplit, ['Month', '# of Member', 'vs LY']);
	} else {
		Createtabulate(Table_filteredData_byMktTyp, ['Month', '# of Member', 'vs BGT', 'vs LY']);
	}
	;
	...
	//Table tab conditional formatting
		...
		if (MembVal == "Air Recruited (Without Split)" ||  MembVal == "Non Air Recruited (Without Split)"  ||  MembVal == "AM.com (Without Split)"){
			$('#card-text-captionVsBGT').hide();
			$('#FigureVsBGT').hide();
			$('#card-text-captionVsBGTFY').hide();
			$('#progresschartdiv').hide();			
		} else {
			$('#card-text-captionVsBGT').show();
			$('#FigureVsBGT').show();
			$('#card-text-captionVsBGTFY').show();
			$('#progresschartdiv').show();
		};
	
	
	
		if (MembSplitVal == 1 & DimVal == 0) {
			$('.imgNewMemberRecuited5050Legend').show();
			$('.imgNewMemberRecuitedwosplitLegend').hide();
		} else if(MembSplitVal == 2 & DimVal == 0){
			$('.imgNewMemberRecuited5050Legend').hide();
			$('.imgNewMemberRecuitedwosplitLegend').show();
		} else {
			$('.imgNewMemberRecuited5050Legend').hide();
			$('.imgNewMemberRecuitedwosplitLegend').hide();
		};
...
*/
	...
	ChartLineHeader = "Member Recruitment" + KeyMktName ;
	document.getElementById("ChartLineHeader").innerHTML  = ChartLineHeader;

	ChartAreaHeader = "Member Recruitment By " + DimName;
	document.getElementById("ChartAreaHeader").innerHTML  = ChartAreaHeader;

	ChartPieHeader = "Member Recruitment" + DimName + " Distribution" ;
...
Legend_Member5050_sort = ['Air', 'Non - Air'];
Legend_MemberSplit_sort = ['Air', 'Non - Air', 'AM.com'];
	
	if(DimVal == 0){
		if(MembSplitVal == 1){
			Legend = Legend_Member5050_sort;	
			LegendClass = "col-12";
		} else {
			Legend = Legend_MemberSplit_sort;
			LegendClass = "col-12";		
		}
	} else {
