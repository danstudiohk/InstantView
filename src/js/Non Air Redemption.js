...
var DataPath = "data/csv/Non Air Redemption.csv";//Data Source location
...
//filtered by Market and Category
...
//filtered by Market and without Total Carrier
	...
	Category_sort = ['Air Ancillary', 'Charity', 'iRedeem','Renewal Out','Service Fees','Transfer Out','TRP'];
...
*/	
	...
	,function(d) { return d.ACT_Miles; })*1.1;
...
//Pie Proportion figures
	...
	svgPie_arcs.append("text")
		...
		.attr("font-size",24)
		.attr("transform",function(d){
			var x = svgPie_arcPath.centroid(d)[0] * 3.5;
...
*/
	function ChartTitleParsing() {
		...
		if (MetVal == "Miles") {
			...
			MetValName = " (Mileages Sales)";
		};
		if (CatVal == "All Categories") {
			CatValName = "Non Air Redemption" ;
		} else {
			CatValName = CatVal;
		};
		if (DimVal == 0) {
	...
	ChartLineHeader = CatValName + " Growth" + KeyMktName;
	document.getElementById("ChartLineHeader").innerHTML  = ChartLineHeader;
	
	ChartAreaHeader = CatValName + " Growth by " + DimName + KeyMktName;
	document.getElementById("ChartAreaHeader").innerHTML  = ChartAreaHeader;
	
	ChartPieHeader = CatValName + " " + DimName + " Distribution";
...
Legend_Category_sort = ['Air Ancillary', 'Charity', 'iRedeem','Renewal Out','Service Fees','Transfer Out','TRP'];
